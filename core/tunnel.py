"""
Tunnel Manager - Alternatives gratuites sans compte
Supporte: localhost.run (SSH), bore, rathole, frp
...
class TunnelProvider(Enum):
    ...
    SERVEO = "serveo"  # SSH, gratuit, sans compte
...
class TunnelConfig:
    ...
    provider: TunnelProvider = TunnelProvider.LOCALHOST_RUN
...
class TunnelManager:
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    data = json.load(f)
                    self.config.provider = TunnelProvider(data.get('provider', 'localhost.run'))
        ...
        providers = [
            {
                ...
                "name": "localhost.run",
                "description": "Tunnel SSH gratuit, aucun compte requis",
            ...
            },
            {
                ...
                "name": "Bore",
                "description": "Tunnel TCP léger en Rust, aucun compte requis",
    ...
    def start(self, provider: Optional[str] = None, port: int = 25565) -> Dict[str, Any]:
        """Démarre le tunnel"""
        with self._lock:
        ...
        try:
            if self.config.provider == TunnelProvider.LOCALHOST_RUN:
    ...
    def _start_localhost_run(self) -> Dict[str, Any]:
        """Démarre un tunnel via localhost.run (SSH)"""
